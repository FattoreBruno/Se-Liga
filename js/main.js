...
function updateRssStatus(message, isError = false) {
    ...
    if (statusDisplay) { try { document.getElementById("debug-stage").textContent = "STATUS: Main.js - updateRssStatus: #rss-status-display FOUND"; } catch(e){console.error("Debug fail:",e)} } else { try { document.getElementById("debug-stage	extContent = "STATUS: Main.js - updateRssStatus: #rss-status-display NOT FOUND!"; } catch(e){console.error("Debug fail:",e)} }
    if (statusDisplay) {
        ...
        try { document.getElementById("debug-stage").textContent = "STATUS: Main.js - updateRssStatus - #rss-status-display NOT FOUND (when trying to set msg): " + (message ? String(message).substring(0,50) : "null"); } catch(e){}
    }
}
...
window.initializeHomePage = function() {
    ...
    console.log('initializeHomePage called from router (full main.js).');
...
function populateHTML(episodes) {
    ...
    updateRssStatus(null);
    if (episodes.length > 0) {
        populateFeaturedEpisode(episodes[0]);
    } else {
        populateFeaturedEpisode(null);
    }
    const episodesForDestaques = episodes.slice(1, 4); populateDestaques(episodesForDestaques);
    const episodesForMaisOuvidos = episodes.slice(4, 7); populateMaisOuvidos(episodesForMaisOuvidos);
    const episodesForMaisEpisodios = episodes.slice(7, 11); populateMaisEpisodios(episodesForMaisEpisodios);
}
